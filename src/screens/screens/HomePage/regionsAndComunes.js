...
let indexRegions = 0;
export const regions = regionsAndComunes.regions.map(region => ({
  key: indexRegions++,
  label: region.name,
}));
const comunes2 = {};

regionsAndComunes.regions.forEach(region => {
  let indexComunes = 0;
  comunes2[`${region.name}`] = region.communes.map(comune => ({
    key: indexComunes++,
    label: comune.name,
  }));
});

export const comunes = comunes2;
